...
def argparser():
    ...
    parser.add_argument(
        '-f', '--files', nargs='+', help="space seperated values of test run JSON files generated from ci_monitor tool")
    parser.add_argument(
        '-o', '--output', help="location to write output file", default=f"./{datetime.today().strftime('%Y%m%d')}.json")
...
def get_test_failure_profile(content: str, profile: str):
    ...
    if content:
        linkto_logs = '[Link to logs|' + \
            content.groups()[0] + ']|' + profile
    else:
                    ...
                    if report_struct.get(owner, 0):
                        if report_struct[owner].get(automation_script, 0):
                            if report_struct[owner][automation_script].get(id, 0):
                                ...
                                report_struct[owner][automation_script].update({id: [
                                    linkto_logs]})
                        else:
